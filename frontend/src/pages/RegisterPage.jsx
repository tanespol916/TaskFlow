    ...
    <div className="flex min-h-screen items-center justify-center bg-bg p-4">
      ...
      <div className="w-full max-w-[380px]">
        <div className="mb-8 text-center">
          ...
          <p className="mt-1.5 text-sm text-text2">เริ่มต้นใช้งานฟรี</p>
        </div>

        <div className="rounded-[20px] border border-borderc bg-surface p-7 shadow-soft">
          <h2 className="mb-5 text-lg font-semibold text-textc">สมัครสมาชิก</h2>
          <form onSubmit={handleSubmit} className="flex flex-col gap-3.5">
            {[
              { label: 'ชื่อผู้ใช้', field: 'username', type: 'text', placeholder: 'username' },
              { label: 'อีเมล', field: 'email', type: 'email', placeholder: 'your@email.com' },
              { label: 'ยืนยันรหัสผ่าน', field: 'confirm', type: 'password', placeholder: '••••••••' },
            ].map(({ label, field, type, placeholder }) => (
              <div key={field} className="flex flex-col gap-1.5">
                <label className="text-[13px] font-medium text-textc">{label}</label>
                <input
                  type={type}
                  ...
                  onChange={set(field)}
                  required
                  className="w-full rounded-xl border border-borderc bg-surface2 px-3.5 py-2.5 text-sm text-textc outline-none transition placeholder:text-text2 focus:border-primary focus:ring-4 focus:ring-indigo-500/15"
                />
              </div>
            ))}
            ...
            <div className="flex flex-col gap-1.5">
              ...
              <div className="relative">
                ...
                />
                <button type="button" onClick={() => setShowPw((v) => !v)} className="absolute right-3 top-1/2 flex -translate-y-1/2 items-center text-text2">
          ...
          <p className="mt-5 text-center text-[13px] text-text2">
            มีบัญชีแล้ว?{' '}
            <Link to="/login" className="font-semibold text-primary no-underline hover:text-primary-hover">เข้าสู่ระบบ</Link>
          </p>
        </div>
      </div>
    </div>
